...
def _exam_payload_rule_findings(
    ...
    for event in events:
        ...
        if event_type != "exam_result":
            continue
        marker = _exam_marker(event.payload)
        value = _exam_value(event.payload)
        if marker is None or value is None:
            continue
        grouped.setdefault(marker, []).append(event)

    for marker, marker_events in grouped.items():
        ordered = sorted(marker_events, key=lambda item: item.occurred_at, reverse=True)
        if len(ordered) < 2:
            continue
        current = ordered[0]
        previous = ordered[1]
        if baseline_entry is not None and current.occurred_at <= baseline_entry.occurred_at:
            continue
        current_value = _exam_value(current.payload)
        previous_value = _exam_value(previous.payload)
        ...
        finding = _exam_delta_finding(
            ...
            unit=str(current.payload.get("unit") or previous.payload.get("unit") or ""),
        )
        if finding:
