...
def test_patient_ai_suggestions_abac_denies_without_active_relationship(
    client: TestClient,
    auth_headers,
    ...
    assert "ai.suggestions.read" not in actions
    assert "access_context.passive_decision" not in actions


def test_patient_ai_suggestions_abac_allows_active_relationship(
    client: TestClient,
    auth_headers,
    create_patient_for_permissions,
    audit_events_for_patient,
    ...
    actions = [event["action"] for event in audit_events_for_patient(patient_id)]
    assert "ai.suggestions.read" in actions
