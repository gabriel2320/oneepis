...
def test_patient_clinical_risks_abac_denies_without_active_relationship(
    client: TestClient,
    auth_headers,
    ...
    list_response = client.get(f"/api/v1/patients/{patient_id}/clinical-risks", headers=auth)
    ...
    assert "clinical_risks.read" not in actions
    assert "clinical_risk.read" not in actions


def test_patient_clinical_risks_abac_allows_active_relationship(
    client: TestClient,
    auth_headers,
    create_patient_for_permissions,
) -> None:
    ...
    assert [item["id"] for item in list_response.json()] == [risk["id"]]
