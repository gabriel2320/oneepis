...
function parseCapabilities() {
  ...
  const functionCallCapabilities = parseCapabilityCalls(source);
  if (functionCallCapabilities.length > 0) {
    return functionCallCapabilities;
  }
...
function parseCapabilityCalls(source) {
  const matches = [...source.matchAll(/capability\(([\s\S]*?)\),/g)];
  return matches
    .map((match) => {
      const strings = [...match[1].matchAll(/"([^"]*)"/g)].map((item) => item[1]);
      const route = strings[0];
      if (!route) {
        return null;
      }
      const status = strings[3] ?? "";
      const paperPolicy = strings[8] ?? "none";
      return {
        route,
        lifecycle: status === "bloqueada" ? "blocked" : "complete",
        screenKind: route.startsWith("/print/") ? "print" : "app",
        paperPolicy,
        clinicalUse: status === "bloqueada" || paperPolicy === "bloqueado" ? "blocked" : "development-only",
      };
    })
    .filter(Boolean);
}
