...
const failedRows = rows.filter((row) => row.missing_in_ts.length > 0);
const report = {
  generated_at: new Date().toISOString(),
  scope: "openapi_to_frontend_type_contracts",
  summary: {
    contracts_checked: rows.length,
    drifted_contracts: failedRows.length,
  },
  rows,
};
...
if (failedRows.length > 0) {
  ...
  for (const row of failedRows) {
    console.error(`- ${row.schema} -> ${row.ts_type}: missing ${row.missing_in_ts.join(", ")}`);
  }
  process.exit(1);
}

console.log(`Contract drift check passed: ${rows.length} frontend contracts match OpenAPI fields.`);
...
function compareContract(contract) {
  ...
  return {
    ...
    ts_fields: tsFields,
    missing_in_ts: openapiFields.filter((field) => !tsFieldSet.has(field)),
