...
const readLikePosts = new Set([
  ...
  "patient_assistant_chart.py POST /{patient_id}/assistant/chart",
  "patient_assistant_correlation.py POST /{patient_id}/assistant/correlate",
  "patient_medications.py POST /{patient_id}/medications/validate-draft",
]);
...
if (warningRows.length > 0) {
  console.warn(`Permission guard report: ${warningRows.length} route(s) without 403 test evidence.`);
  for (const row of warningRows) {
    console.warn(`- ${row.route_file} ${row.method} ${row.path}: ${row.warnings.join(", ")}`);
  }
}

console.log(
  `Permission guard passed: ${rows.length} mutating clinical routes, no critical gaps.`,
);
...
function actorOrAccessDep(body) {
  ...
  if (body.match(/\buser:\s*CurrentUserDep\b/)) {
