...
- Existe enforcement dev-only para `GET /api/v1/patients/{patient_id}`,
...
- Los headers contextuales siguen rechazados y auditados; `break_glass_enabled`,
  ...
  `False`.
- Falta enforcement runtime productivo, cobertura de enforcement en AI/Assistant
  Read, clinical orders, vitals, labs, risks, entries/events/encounters,
  indices globales, motivo de acceso operativo, revision break-glass, UI/flujo
  institucional, ownership clinico/legal y pruebas E2E de denegacion.

## Proximo paso recomendado

Cerrar primero los bypasses de lectura patient-scoped en AI/Assistant antes de
abrir pantallas, modulos HIS, receta, firma, MAR, adjuntos, consentimientos o IA
externa. Crear issues separados a partir de estos IDs solo despues de una
revision humana del checklist. Hasta entonces, este documento es la fuente
versionada de no-produccion.
